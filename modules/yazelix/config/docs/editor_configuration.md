...
**Requirements:**
- ⚠️ **Helix must be in PATH** - Ensure your preferred `hx` is available
...
### Helix Command Key Remaps

If you remap Helix `command_mode` (for example, `";" = "command_mode"`), update your Yazelix config so sidebar openers use the right key:
...
Home Manager users can set `programs.yazelix.helix_command_key = ";";`.
