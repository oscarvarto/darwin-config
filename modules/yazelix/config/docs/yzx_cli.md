...
- Default: Drop into your configured shell
- `--no-shell`: Stay in current shell (no shell switch)
- `--command CMD`: Run a single command with Yazelix env vars set and exit
...
### `yzx update`
Update guidance (Yazelix is managed by the main Nix configuration)
...
yzx profile --cold            # Profile cold start (informational)
```
