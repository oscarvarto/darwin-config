...
def get_dynamic_overrides [] {
    ...
    let rounded_value = if ($rounded | describe) == "bool" {
        if $rounded { "true" } else { "false" }
    } else {
        let rounded_str = ($rounded | into string)
        if ($rounded_str | str starts-with "false") { "false" } else { "true" }
    }

    let theme = ($config | get -o zellij_theme | default "default")
    let theme_line = if $theme != "default" {
        $"theme \"($theme)\""
    } else {
        ""
    }
...
export def generate_merged_zellij_config [yazelix_dir: string] {
    ...
    if ($source_layouts_dir | path exists) {
        # Get widget_tray from yazelix config
        let config = (try {
            parse_yazelix_config
        } catch {
            {widget_tray: ["layout", "editor", "shell", "term", "cpu", "ram"]}
        })
        let widget_tray = ($config | get -o widget_tray | default ["layout", "editor", "shell", "term", "cpu", "ram"])

        # Copy layouts to merged config directory with widget tray substitution
