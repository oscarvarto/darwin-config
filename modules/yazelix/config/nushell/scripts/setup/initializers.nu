...
def main [yazelix_dir: string, recommended: bool, atuin_enabled: bool, shells_to_configure_str: string] {
    ...
    let results = ($shells | each { |shell|
        ...
        # For Nushell: preserve PATH entries from the parent shell
        let nushell_path_preservation = if ($shell.name == "nu") {
            [
                ""
                "# Preserve PATH from the parent shell (includes packs like python, rust, etc.)"
        ...
        let nushell_path_restoration = if ($shell.name == "nu") {
            [
                ...
                ""
                "# Restore PATH entries that may have been lost during initialization"
                "# Merge initial_path with current PATH, preserving store paths"
