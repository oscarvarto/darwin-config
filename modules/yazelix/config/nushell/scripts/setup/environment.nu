...
# Called during Yazelix startup to reduce complexity
...
def main [
    ...
    # Set permissions when config dir is writable (skip for managed configs)
    if not $env_info.read_only_config {
    ...
    show_welcome $skip_welcome_screen $quiet_mode $ascii_art_mode $welcome_message $log_dir $colors
}
