...
def get_ghostty_shader_root [] {
    let config_dir = ($YAZELIX_CONFIG_DIR | path expand)
    $config_dir | path join "configs" "terminal_emulators" "ghostty" "shaders"
}

def expand_shader_path [shader: string] {
    if ($shader | is-empty) {
        ""
    } else if ($shader | str starts-with "./") {
        let rel_path = ($shader | str replace "./" "")
        (get_ghostty_shader_root | path join $rel_path)
    } else {
        $shader
    }
}
...
def get_cursor_trail_shader [cursor_trail: string] {
    let shader = ($CURSOR_TRAIL_SHADERS | get -o $cursor_trail | default $CURSOR_TRAIL_SHADERS.blaze)
    expand_shader_path $shader
}
...
def build_cursor_trail [cursor_trail: string] {
    if $cursor_trail == "none" {
        let default_shader = get_cursor_trail_shader "blaze"
        $"# custom-shader = ($default_shader)"
...
config.default_prog = {\"bash\", \"-l\", \"-c\", \"nu --login ~/.config/yazelix/nushell/scripts/core/start_yazelix.nu\"}
