...
def main [
    ...
    # Parse config (auto-creates yazelix.toml if missing)
    ...
    # Set NUSHELL_THEME=dark so nushell uses dark theme inside yazelix
    if $terminal_info.use_wrapper {
        mut env_block = {
        ...
        if ($env.YAZELIX_LAUNCH_SHELL? | is-not-empty) {
            $env_block = ($env_block | upsert YAZELIX_LAUNCH_SHELL $env.YAZELIX_LAUNCH_SHELL)
        }
        if $verbose_mode {
