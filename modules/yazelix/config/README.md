...
- This repository integrates Yazelix directly into the main Nix configuration. There is no separate devenv workflow here.
...
**Disk Space Requirements**: Default installation requires **~4.7GB** (Nix ~2.5GB + Yazelix packages ~2.2GB)
...
5. Copy terminal config (optional because yazelix offers built-in version for all supported terminals)
...
- **System Helix**: `editor_command = "hx"` - Uses Helix from PATH with full integration
...
If you prefer a raw environment shell without launching Yazelix, use `yzx env --no-shell`.
...
Want to use Yazelix tools (Nushell, zoxide, starship, lazygit) inside your editor? VS Code and Cursor work seamlessly with `yzx env`.
...
For more advanced integration options, see our [VS Code and Cursor terminal integration guide](./docs/editor_terminal_integration.md).
