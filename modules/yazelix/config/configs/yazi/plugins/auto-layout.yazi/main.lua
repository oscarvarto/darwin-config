...
--
-- Toggle preview mode with keybinding (add to keymap.toml):
--   { on = ["T"], run = "plugin auto-layout --args=toggle", desc = "Toggle preview pane" }
...
-- --- State ---
-- State file to persist preview mode across Yazi restarts
local state_file = "/tmp/yazelix-yazi-preview-mode"

-- Read preview mode state from file
local function read_preview_state()
  local file = io.open(state_file, "r")
  if file then
    local content = file:read("*a")
    file:close()
    return content:match("browse") ~= nil
  end
  return false  -- Default: sidebar mode (no preview)
end

-- Write preview mode state to file
local function write_preview_state(browse_mode)
  local file = io.open(state_file, "w")
  if file then
    file:write(browse_mode and "browse" or "sidebar")
    file:close()
  end
end

-- Preview mode: false = sidebar (1-column), true = browse (3-column with preview)
local preview_mode_enabled = read_preview_state()
-- --- End State ---
...
local function in_sidebar_pane()
  local pane_name = os.getenv("ZELLIJ_PANE_NAME") or os.getenv("ZELLIJ_PANE_TITLE") or ""
  return pane_name == "sidebar"
end
...
local function auto_layout_override(self)
  ...
  local success, result = pcall(function()
    ...
    -- Check if we're in sidebar pane AND preview mode is not enabled
    if in_sidebar_pane() and not preview_mode_enabled then
      layout_type = "sidebar-1-column"
      constraints = {
        ui.Constraint.Ratio(0, ratios.all),
        ui.Constraint.Ratio(ratios.all, ratios.all),
        ui.Constraint.Ratio(0, ratios.all),
      }
    elseif w > plugin_config.breakpoint_large or (in_sidebar_pane() and preview_mode_enabled) then
      -- Use 3-column layout if wide enough OR if preview mode is enabled in sidebar
    ...
    log_info(string.format("Applying %s layout (preview_mode=%s).", layout_type, tostring(preview_mode_enabled)))
...
local function toggle_preview_mode()
  preview_mode_enabled = not preview_mode_enabled
  write_preview_state(preview_mode_enabled)
  
  -- Notify user
  local mode_name = preview_mode_enabled and "Browse mode (3-column with preview)" or "Sidebar mode (1-column)"
  ya.notify({
    title = "Yazi Layout",
    content = mode_name,
    timeout = 2,
  ...
  -- Force layout refresh by emitting a resize event
  ya.manager_emit("resize", {})
  
  log_info(string.format("Preview mode toggled to: %s", tostring(preview_mode_enabled)))
end

-- Entry point for plugin commands (called via keybinding)
function M.entry(_, args)
  local action = args[1]
  if action == "toggle" then
    toggle_preview_mode()
  elseif action == "browse" then
    if not preview_mode_enabled then
      preview_mode_enabled = true
      write_preview_state(true)
      ya.notify({ title = "Yazi Layout", content = "Browse mode (3-column with preview)", timeout = 2 })
      ya.manager_emit("resize", {})
    end
  elseif action == "sidebar" then
    if preview_mode_enabled then
      preview_mode_enabled = false
      write_preview_state(false)
      ya.notify({ title = "Yazi Layout", content = "Sidebar mode (1-column)", timeout = 2 })
      ya.manager_emit("resize", {})
    end
  end
end
